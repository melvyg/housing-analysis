# Locating Your Home: An In-Depth Analysis on Housing Locations and Prices in King County, WA

**Author**: Melvin Garcia

## Overview
The aim of this report is to derive insights on the types of location-based features that influence house prices. The data for this report comes from official public records of home sales in King County, Washington State along with demographic data of the area from the National Historical GIS. These data insights are to help our real estate agency stakeholders provide general advice for their clientele on where to potentially buy a house. Location is a feature of interest because there is a identified cohort of home-buyers that are out-of-state and are looking to buy a vacation home. The method to analyze our location based-features and house prices in King County, WA is though regression modeling. Regression modeling will allow us to identify which features have an impact on house prices -- which features matter most, and which features can be ignored. From this analysis, it was was observed that the rating how good the view of the property was and whether a house was located within and outside a range of 45.5 and 47.7 latitude have the most impact on housing prices. From the demographic data, it was also observed that education level, specifically populations of people with a GED or alternative credential and a Masters degree had an impact on housing prices. As a result, we are able to advise the following: living within the specified latitude range increases the house price by $65,910 on average, and the rating of the view of a property increases the house price by $84,910 on average.

## Business Problem
The real estate agency had noticed a growing cohort of clientele inquiring about buying a vacation home in King County, WA. At the same time, a characteristic of these clients are out-of-state likely with limited experience of the landscape and locations in King County. Therefore in order for our real estate agency to provide sound advice on potential locations and their corresponding prices, our stakeholder is interested in location based features and their impact on house prices.

## Data
The data used for this report comes from the official public records of the home sales in the King County area of Washington State. The data contains 21,597 with home sales from May 2014 - May 2015. The data points contained within this dataset range from the number bedrooms, bathroom, the square footage of the interior living space, the year the house was built, and the latitude and longitude of the home. The demographic data are aggragated on a zipcode basis and hence was joined to our sales data using this the key. The demographic data contained data points from median age, median income, total population, racial population, and  education level demographic.
...
This step was confirming the data sources and features to examine -- that is the King County, WA home sales data as well as the NHGIS demographic data while targeting location-centric based features. The next and one of the most important steps to perform is data exploration to examine any obvious correlations between our features and our target of house price, while also being mindful of the features that are potentially relevant to our stakeholders.

**Clean the data**

Another important step to perform that is done in conjunction with data exploration is examining the data quality ie. missing values, mis-inputs, etc. For example, the features missing the most entries included whether a property contained a waterfront and the year the house was renovated. The waterfront feature was more relevant for our analysis, and we dropped year renovated. It was discovered that ~11% of the total rows of the dataset were missing the waterfront feature. As a result, we decided to simply drop the rows missing waterfront reducing our dataset from 21,597 rows to 19,164.
...
In the left image, there appears a strong right skew with the King County house prices. Removing these may help with the issues we saw with the initial linear regression assumption checks.
...
After filtering our dataframe focusing on the range of house prices with the largest residuals, we set up the heatmap correlation matrix to examine additional features that could help with the homoscedasticity assumptions we saw from earlier model iterations. Including features from the census data, we can see that ged_or_alternative_credential_25_years_and_over has a moderately strong negative correlation that we can add.

### OLS Model Summary Output
![graph14](./images/ModelC_Summary.jpg)
In this iteration of modeling, we removed age as a feature, and added whether a house has a view or not to the further investigate if aspects of location can help explain the variance in house prices. Collectively, after removal of age and addition of view, the feature set explains 46% of the variance in house prices.

According to our model:
- A house without regard to the provided features is expected to have a house price of approximately $493,600.
- For an additional km in the distance from downtown, we expect the house price to decrease by $222. However, distance from downtown is also seen to have a p-value larger than our 0.05 threshold, the relationship between distance from downtown and home prices is not statistically significant.
- Having a waterfront increases the house price by $21,610 on average. However, waterfront is also seen to have a p-value larger than our 0.05 threshold, therefore there is no correlation between waterfront and house price.
- As the number of people in a region with a GED or alternative credential while being 25 years of age or older increases, the value of a home decreases on average by $259.
- As the number of people in a region with Masters degree while being 25 years of age or older increases, the value of a home increases by $22 on average.
- By having a house located between 45.5 and 47.7 latitude increases the price of a house by $65,910 on average.
- The quality of the view increases the price of a house on average by $83,830 US dollars
